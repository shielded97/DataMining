...
flags = tf.app.flags
FLAGS = flags.FLAGS
flags.DEFINE_string('model_dir', 'nat_canvas', 'saved model directory')
flags.DEFINE_string('dataset', 'canvas', 'Dataset string.')  # 'cora', 'citeseer', 'pubmed'
...
def hamming(features, support, labels, mask, placeholders):
    ...
    outputs = []
    predictions = []
    pred = tf.arg_max(model.outputs, 1)
    pred = sess.run(pred, feed_dict=feed_dict_val)
    # run some normalizations
    # lower = min(pred)
    # upper = max(pred)
    # preds_norm = [round(6 * ((x - lower) / (upper - lower))) for x in pred]
    final = []
    for truth, x in zip(mask, pred):
        if truth == True:
            final.append(x)
    # # for x in features:
    #     input_array_for_prediction = np.expand_dims(x,axis=0)
    #     pred = tf.arg_max(x, dimension=-1)
    #     outputs.append(pred)
    # for x in outputs:
    #     pred = sess.run(x)
